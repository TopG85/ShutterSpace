...
class CommentForm(forms.ModelForm):
    ...
    def clean_text(self):
        ...
        if len(words) > 0:
            ...
            word_ratio = words.count(most_common_word) / len(words)
            if word_ratio > 0.5 and len(words) > 4:
...
class ProfileForm(forms.ModelForm):
    avatar = CloudinaryFileField(
        ...
        help_text=("Upload a profile photo (JPEG, PNG, or WebP). "
    ...
    hero_image = CloudinaryFileField(
        ...
        help_text=("Upload a header image for your profile "
    ...
    class Meta:
        ...
        widgets = {
            ...
            'bio': forms.Textarea(attrs={
                'class': 'form-control',
                'rows': 4,
                'placeholder': ('Tell us about yourself, your photography '
                                'style, interests...'),
                'maxlength': '500'
            }),
        ...
        help_texts = {
            'display_name': ('How you want to be known on ShutterSpace '
                             '(optional)'),
    ...
    def clean_bio(self):
        ...
        if bio:
            ...
            if len(bio) < 10:
                raise ValidationError(
                    "If you provide a bio, please write at least 10 "
                    "characters to tell us about yourself.")
    ...
    def clean_location(self):
        ...
        if location:
            ...
            if not re.match(r'^[a-zA-Z0-9\s\-,\.]+$', location):
                raise ValidationError(
                    "Location can only contain letters, numbers, spaces, "
    ...
    def clean_instagram(self):
        ...
        if instagram:
            ...
            if len(instagram) > 30:
                raise ValidationError(
                    "Instagram handle cannot exceed 30 characters.")

            # Validate Instagram handle format
            if not re.match(r'^[a-zA-Z0-9_.]+$', instagram):
                raise ValidationError(
                    "Instagram handle can only contain letters, numbers, "
                    "underscores, and periods.")

            if instagram.startswith('.') or instagram.endswith('.'):
                raise ValidationError(
                    "Instagram handle cannot start or end with a period.")

            if '..' in instagram:
                raise ValidationError(
                    "Instagram handle cannot contain consecutive periods.")
    ...
    def clean(self):
        ...
        if display_name and not bio:
            self.add_error(
                'bio',
                "Consider adding a biography to tell others about yourself.")
